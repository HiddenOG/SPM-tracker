...
def park_email(
    *,
    ...
    Always indexes by the base PO number so change-order suffix variants
    are found when the base row is later created.
    """
    client = get_client()
...
def get_parked_for_po(po_number: str) -> list[dict]:
    # Always query by base PO. Use LIKE "base%" to also surface any rows that
    # were parked before this fix using the old "-001" suffixed key.
    client = get_client()
    base = base_po_number(po_number) or po_number
    res = (
        client.table("parked_emails")
        .select("*")
        .like("po_number", f"{base}%")
...
def email_thread_id(msg) -> str | None:
    """Derive a stable thread root ID from standard email threading headers."""
...
def upsert_email(
    *,
    ...
    """Write one email record to the emails table.
    Silently no-ops when neither order_id nor nlng_order_id is known — we only
    store emails that can be linked to an order for the story drawer.
    Never raises: email story is non-critical and must not crash a pipeline step.
    """
    if not order_id and not nlng_order_id:
        return
    if not message_id:
        return
    try:
        row: dict = {"message_id": message_id, "direction": direction}
        if order_id:
            row["order_id"] = order_id
        if nlng_order_id:
            row["nlng_order_id"] = nlng_order_id
        if from_address:
        ...
        if subject:
            row["subject"] = subject
        if body_text:
            row["body_text"] = body_text[:50_000]   # cap to avoid storage bloat
        if received_at:
        ...
        print(f"  ⚠️  emails upsert failed (non-fatal): {_e}")
