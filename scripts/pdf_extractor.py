...
def extract_pdf_with_pdfplumber(pdf_path: str) -> dict:
    ...
    if items_by_line:
        ...
        all_expected = {str(i) for i in range(1, max_line + 1)}
    ...
    # Order-level promised date = earliest promised date across line items.
    ppds = [li["promised_date"] for li in result["line_items"] if li.get("promised_date")]
    result["po_promised_date"] = min(ppds) if ppds else None
