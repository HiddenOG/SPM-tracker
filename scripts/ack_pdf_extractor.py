    ...
    if url_orders:
        ...
        for order in url_orders:
            ...
            if ack_date:
                ...
                no_date += 1
                # Clear the flag so this PDF isn't re-downloaded every cycle.
                # The PDF is static in Supabase Storage — pdfplumber will never
                # find a date on a second attempt. If a new ack PDF arrives later,
                # gmail_ack_listener will set pending_ack_extraction=True again.
                client.table("orders").update({"pending_ack_extraction": False}).eq("id", order["id"]).execute()
                print(f"⏳ PO {po}: ack PDF downloaded but no date found — extraction flag cleared")
