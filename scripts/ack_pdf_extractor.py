...
def _safe_url(url: str) -> str:
    """Encode spaces and special chars in the URL path so urllib can fetch it."""
    parsed = urlparse(url)
    return urlunparse(parsed._replace(path=quote(parsed.path, safe="/")))


def _claude_ack_date(pdf_path: str, po: str) -> str | None:
    """Claude API fallback for image-based ePurchase PDFs where pdfplumber finds no text."""
    try:
        import anthropic
        from pdf_extractor import parse_gep_date
        ...
        msg = client.messages.create(
            ...
            messages=[{
                ...
                "content": [
                    ...
                    {
                        "type": "text",
                        "text": "Find the 'Supplier Acknowledged on' date. Reply with only the date in YYYY-MM-DD format, or 'none' if not present.",
                    },
                ],
            }]
        )
        ...
        if re.match(r"^\d{4}-\d{2}-\d{2}$", result):
            return result
        m = re.search(r"(\d{1,2}/\d{1,2}/\d{4})", result)
        if m:
            return parse_gep_date(m.group(1))
    except Exception as e:
        print(f"   ⚠️  PO {po}: Claude ack extraction failed — {e}")
    return None
...
def _download_and_extract(url: str, po: str) -> str | None:
    ...
    try:
        with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tmp:
            tmp_path = tmp.name
        urllib.request.urlretrieve(_safe_url(url), tmp_path)
        # pdfplumber first (works for native GEP PDFs with a text layer)
        date = extract_ack_date(tmp_path)
        if date:
            return date
        # Claude fallback for image-based ePurchase PDFs (no text layer)
        return _claude_ack_date(tmp_path, po)
    except Exception as e:
