...
def run() -> None:
    ...
    try:
        ...
        uids = imap.search(["FROM", SPM_SENDER, "SUBJECT", "for Delivery"])
        ...
        for i in range(0, len(uids), FETCH_BATCH):
            ...
            for uid in chunk:
                ...
                # "Request for Delivery Date Extension" is about extending the
                # deadline with Chevron, NOT requesting physical dispatch.
                if _re.search(r'delivery\s+date|date\s+ext', subject, _re.IGNORECASE):
                    print(f"  ⏭ Skipping date-extension email: {subject[:60]}")
                    continue
