...
def is_spm_delivery_approval_subject(subject: str) -> bool:
    ...
    "Request for Delivery Date Extension" looks similar but is about pushing back
    Chevron's deadline — NOT requesting physical dispatch. Exclude it explicitly.
    """
    if re.search(r'delivery\s+date|date\s+ext', subject, re.IGNORECASE):
        return False
    return bool(re.search(r"requested?\s+for\s+delivery", subject, re.IGNORECASE))
...
def process_nlng_spm_po_send(client_db, subject: str, email_date: str,
    ...
    if stamped_ids:
        to_addr = decode_mime_words(msg.get("To", "")) if msg is not None else None
        sync.upsert_email(
            message_id=message_id,
            nlng_order_id=stamped_ids[0],
            direction="out",
            from_address=sender,
            to_address=to_addr,
            subject=subject,
            body_text=get_email_body_text(msg) if msg is not None else None,
            received_at=email_date,
            in_reply_to=msg.get("In-Reply-To") if msg is not None else None,
            thread_id=sync.email_thread_id(msg) if msg is not None else None,
        )
...
def process_message(client_db, msg_data: dict) -> None:
    ...
    if pdf_path:
        # Upload ack PDF immediately after saving — same pattern as imap_listener
        # does for PO PDFs. This runs before Claude so the PDF always lands in
        # Supabase Storage even when Claude is unavailable or out of credits.
        # ack_pdf_extractor.py will then extract the date via pdfplumber.
        _ack_order = find_order_by_po_number(po_hint)
        if _ack_order:
            _ack_url = upload_ack_pdf(pdf_path, po_hint)
            if _ack_url:
                get_client().table("orders").update({
                    "ack_pdf_url": _ack_url,
                    "pending_ack_extraction": True,
                }).eq("id", _ack_order["id"]).execute()

        try:
            ack_info = extract_ack_info_with_claude(pdf_path)
        except Exception as e:
            ack_info = None
            notes.append(f"Claude extraction failed (non-fatal): {e}")
            print(f"⚠️  Claude unavailable for {subject} — ack_pdf_extractor will extract date via pdfplumber.")
    ...
    if is_routing:
        ...
        if nlng_po:
            ...
            if nlng_order:
                ...
                nlng_matched_order_id = nlng_order["id"]
                result = "created_order"
    ...
    if matched_order_id or nlng_matched_order_id:
        direction = "out" if is_routing else "in"
        sync.upsert_email(
            message_id=message_id,
            order_id=matched_order_id,
            nlng_order_id=nlng_matched_order_id,
            direction=direction,
            from_address=sender,
            to_address=decode_mime_words(msg.get("To", "")),
            subject=subject,
            body_text=routing_body or get_email_body_text(msg),
            received_at=email_date,
            in_reply_to=msg.get("In-Reply-To"),
            thread_id=sync.email_thread_id(msg),
        )
