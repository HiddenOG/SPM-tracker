...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _serve_alerts(self) -> None:
        ...
        try:
            ...
            chev = (get_client().table("orders")
                    .select("id,buyer_po_number,buyer_name,promised_date,required_delivery_date,"
                            "delivered_at,overall_status,notification_received_at,delivery_requested_at")
                    .execute().data or [])
            nlng = (get_client().table("nlng_orders")
                    .select("id,po_number,promised_date,required_delivery_date,"
                            "delivered_at,overall_status,notification_received_at")
                    .execute().data or [])
