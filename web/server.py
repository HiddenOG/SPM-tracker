...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _build_email_context(self, po_number: str, so_number: str = "") -> str:
        ...
        Samples first 3 (oldest), middle 3, and last 3 (newest) so the AI sees
        the beginning, middle, and current state of the thread.
        """
        ...
        if n <= 9:
            ...
            mid = n // 2
            first  = emails[:3]
            middle = emails[max(0, mid - 1) : mid + 2]  # 3 around the midpoint
            last   = emails[-3:]
            seen: set[str] = set()
            sampled = []
            for e in first + middle + last:
                key = e.get("message_id") or str(id(e))
                if key not in seen:
                    seen.add(key)
                    sampled.append(e)
            sampled.sort(key=lambda e: e.get("received_at") or "")
        ...
        for e in sampled:
            ...
            body  = (e.get("body_text") or "").strip()[:500]
    ...
    def _handle_email_chat(self) -> None:
        try:
            ...
            # Keep only the last 6 messages (3 exchanges) — shorter memory keeps
            # the AI focused on the current question rather than drifting on old context.
            messages = messages[-6:]
