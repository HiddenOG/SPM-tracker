...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _fetch_gmail_emails(self, po_number: str, so_number: str = "") -> list:
        ...
        try:
            ...
            subj_uids = set(imap.search(["SUBJECT", po_number]))

            # Chevron PO numbers have leading zeros (e.g. 0061378648). Some emails
            # reference the same PO without them (61378648). Search both forms so
            # no part of the thread is missed.
            stripped_po = po_number.lstrip("0")
            if stripped_po and stripped_po != po_number:
                try:
                    subj_uids.update(imap.search(["SUBJECT", stripped_po]))
                except Exception:
                    pass

            # For NLNG orders: also search by SO number in the full body so we
            # catch combined shipping emails where our PO is listed inside the
            # body of a thread whose subject belongs to a different order.
            if so_number:
                try:
                    so_uids = imap.search(["X-GM-RAW", f'"{so_number}"'])
                    subj_uids.update(so_uids)
                except Exception:
                    pass
            ...
            # Expand to full threads so all replies are included even if the
            # subject drifted and no longer contains the PO number.
            thread_data = imap.fetch(list(subj_uids), ["X-GM-THRID"])
            thread_ids  = {v[b"X-GM-THRID"] for v in thread_data.values() if b"X-GM-THRID" in v}
            all_uids: set[int] = set(subj_uids)
            ...
            results = []
            for uid in sorted(all_uids)[:200]:
                try:
                    data = imap.fetch([uid], ["RFC822"])
                    raw  = (data.get(uid) or {}).get(b"RFC822")
                    if not raw: continue
    ...
    def _serve_email_summary(self) -> None:
        try:
            ...
            for chunk in self._groq_stream([{"role": "user", "content": prompt}], max_tokens=300):
    ...
    def _build_email_context(self, po_number: str, so_number: str = "") -> str:
        ...
        lines = []
        for e in emails[:30]:  # cap at 30 — older emails rarely matter for AI context
            ...
            body  = (e.get("body_text") or "").strip()[:250]
            lines.append(f"[{date}] {dir_} | From: {frm} | Subject: {subj}\n{body}")
        return "\n\n---\n\n".join(lines)
    ...
    def _groq_stream(self, messages: list, max_tokens: int = 400):
    ...
    def _handle_email_chat(self) -> None:
        try:
            ...
            # Cap history to last 20 turns to keep token cost bounded
            ...
            system_msg  = {
                ...
                "content": (
                    ...
                    "Answer questions concisely using the company background above "
                    "and the email thread below. Use your knowledge of the SPM pipeline "
                    "to explain what stage the order is at and what the next step should be. "
                    "If something is not in the emails, say so — do not invent details.\n\n"
                    f"EMAIL THREAD:\n{thread_text}"
                ),
            }
            self._start_sse()
